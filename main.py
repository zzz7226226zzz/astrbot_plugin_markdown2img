...
class MarkdownConverterPlugin(Star):
    ...
    async def on_llm_req(self, event: AstrMessageEvent, req: ProviderRequest):
        ...
        # 目标：只把“复杂/排版敏感”的片段转图片，普通叙述仍保持纯文本。
        # 规则：允许输出纯文本 + 若干段 <md>...</md>，插件只会把 <md> 内渲染为图片。
        instruction_prompt = """
你正在响应一个 `/md` 指令，用户希望：只有复杂内容生成图片，其它内容保持纯文本。

请遵守以下规则：
1) 你可以输出普通纯文本。
2) 仅当出现复杂排版时才使用 `<md>...</md>` 包裹，例如：表格、代码块、复杂列表、长公式/推导、多级小标题、需要严格对齐的内容。
3) `<md>` 与 `</md>` 必须成对出现，且不要嵌套。
4) `<md>` 标签外仍可有解释性文字（这部分会以纯文本发送）。
5) 如果整段都不复杂，可以完全不使用 `<md>`。
